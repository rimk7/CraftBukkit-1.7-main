...
public class WorldGenBigTree extends WorldGenTreeAbstract {
    ...
    void a() {
        ...
        int i = (int) (1.382D + FastMath.pow(this.k * (double) this.e / 13.0D, 2.0D));
        ...
        while (i1 >= 0) {
            ...
            if (f < 0.0F) {
                ...
                for (double d0 = 0.5D; j1 < i; ++j1) {
                    ...
                    int k1 = (int) FastMath.floor(d1 * FastMath.sin(d2) + (double) this.d[0] + d0);
                    int l1 = (int) FastMath.floor(d1 * FastMath.cos(d2) + (double) this.d[2] + d0);
                    int[] aint1 = new int[] { k1, j, l1};
                    int[] aint2 = new int[] { k1, j + this.n, l1};

                    if (this.a(aint1, aint2) == -1) {
                        int[] aint3 = new int[] { this.d[0], this.d[1], this.d[2]};
                        double d3 = FastMath.sqrt(FastMath.pow((double) FastMath.abs(this.d[0] - aint1[0]), 2.0D) + FastMath.pow((double) FastMath.abs(this.d[2] - aint1[2]), 2.0D));
                        double d4 = d3 * this.i;
    ...
    void a(int i, int j, int k, float f, byte b0, Block block) {
        ...
        for (aint1[b0] = aint[b0]; i1 <= l; ++i1) {
            ...
            while (j1 <= l) {
                double d0 = FastMath.pow((double) FastMath.abs(i1) + 0.5D, 2.0D) + FastMath.pow((double) FastMath.abs(j1) + 0.5D, 2.0D);
    ...
    float a(int i) {
        if ((double) i < (double) ((float) this.e) * 0.3D) {
            ...
            if (f1 == 0.0F) {
                ...
                f2 = (float) FastMath.sqrt(FastMath.pow((double) FastMath.abs(f), 2.0D) - FastMath.pow((double) FastMath.abs(f1), 2.0D));
            }
    ...
    void a(int[] aint, int[] aint1, Block block) {
        ...
        for (b1 = 0; b0 < 3; ++b0) {
            ...
            if (FastMath.abs(aint2[b0]) > FastMath.abs(aint2[b1])) {
                b1 = b0;
            }
        }
        ...
        if (aint2[b1] != 0) {
            ...
            for (int j = aint2[b1] + b4; i != j; i += b4) {
                aint3[b1] = (int) FastMath.floor((double) (aint[b1] + i) + 0.5D);
                aint3[b2] = (int) FastMath.floor((double) aint[b2] + (double) i * d0 + 0.5D);
                aint3[b3] = (int) FastMath.floor((double) aint[b3] + (double) i * d1 + 0.5D);
                byte b5 = 0;
                int k = FastMath.abs(aint3[0] - aint[0]);
                int l = FastMath.abs(aint3[2] - aint[2]);
                int i1 = FastMath.max(k, l);
    ...
    int a(int[] aint, int[] aint1) {
        ...
        if (aint2[b1] == 0) {
            ...
            for (j = aint2[b1] + b4; i != j; i += b4) {
                aint3[b1] = aint[b1] + i;
                aint3[b2] = (int) FastMath.floor((double) aint[b2] + (double) i * d0);
                aint3[b3] = (int) FastMath.floor((double) aint[b3] + (double) i * d1);
