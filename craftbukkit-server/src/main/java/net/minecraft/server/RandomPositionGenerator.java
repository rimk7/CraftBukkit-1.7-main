...
public class RandomPositionGenerator {
    ...
    private static Vec3D c(EntityCreature entitycreature, int i, int j, Vec3D vec3d) {
        ...
        if (entitycreature.bY()) {
            double d0 = (double) (entitycreature.bV().e((int) FastMath.floor(entitycreature.locX), (int) FastMath.floor(entitycreature.locY), (int) FastMath.floor(entitycreature.locZ)) + 4.0F);
        ...
        for (int j1 = 0; j1 < 10; ++j1) {
            ...
            if (vec3d == null || (double) k1 * vec3d.a + (double) i2 * vec3d.c >= 0.0D) {
                ...
                int k1Mod = k1 + (int) FastMath.floor(entitycreature.locX);
                int l1Mod = l1 + (int) FastMath.floor(entitycreature.locY);
                int i2Mod = i2 + (int) FastMath.floor(entitycreature.locZ);
