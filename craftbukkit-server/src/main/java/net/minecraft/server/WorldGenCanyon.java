...
public class WorldGenCanyon extends WorldGenBase {
    ...
    protected void a(long i, int j, int k, Block[] ablock, double d0, double d1, double d2, float f, float f1, float f2, int l, int i1, double d3) {
        ...
        for (; l < i1; ++l) {
            double d6 = 1.5D + (double) (FastMath.sin((float) l * 3.1415927F / (float) i1) * f * 1.0F);
            double d7 = d6 * d3;

            d6 *= (double) random.nextFloat() * 0.25D + 0.75D;
            d7 *= (double) random.nextFloat() * 0.25D + 0.75D;
            float f6 = (float) FastMath.cos(f2);
            float f7 = (float) FastMath.sin(f2);

            d0 += (double) (FastMath.cos(f1) * f6);
            d1 += (double) f7;
            d2 += (double) (FastMath.sin(f1) * f6);
            f2 *= 0.7F;
            f2 += f4 * 0.05F;
            f1 += f3 * 0.05F;
            f4 *= 0.8F;
            ...
            if (flag || random.nextInt(4) != 0) {
                ...
                if (d0 >= d4 - 16.0D - d6 * 2.0D && d2 >= d5 - 16.0D - d6 * 2.0D && d0 <= d4 + 16.0D + d6 * 2.0D && d2 <= d5 + 16.0D + d6 * 2.0D) {
                    int l1 = (int) FastMath.floor(d0 - d6) - j * 16 - 1;
                    int i2 = (int) FastMath.floor(d0 + d6) - j * 16 + 1;
                    int j2 = (int) FastMath.floor(d1 - d7) - 1;
                    int k2 = (int) FastMath.floor(d1 + d7) + 1;
                    int l2 = (int) FastMath.floor(d2 - d6) - k * 16 - 1;
                    int i3 = (int) FastMath.floor(d2 + d6) - k * 16 + 1;
